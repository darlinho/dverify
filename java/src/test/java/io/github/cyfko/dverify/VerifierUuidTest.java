...
public class VerifierUuidTest {
    ...
    @Test
    public void sign_method_with_valid_data_should_returns_token() throws JsonEncodingException {
        UserData data = new UserData("john.doe@example.com");
        Duration duration = Duration.ofHours(2);

        String token = signer.sign(data, duration, TokenMode.uuid);
    ...
    public void sign_method_with_invalid_data_should_throws_exception() {
        ...
        Assertions.assertThrows(IllegalArgumentException.class, () -> signer.sign(invalidData, duration, TokenMode.uuid));
    }
    ...
    public void sign_method_with_expired_duration_should_throws_exception() {
        ...
        Assertions.assertThrows(IllegalArgumentException.class, () -> signer.sign(data, duration, TokenMode.uuid));
    }

    @Test
    public void sign_valid_data_should_returns_token() throws JsonEncodingException {
        UserData data = new UserData("john.doe@example.com");
        Duration duration = Duration.ofHours(2);
    ...
    public void sign_invalid_data_should_throws_exception() {
        ...
        assertThrows(IllegalArgumentException.class, () -> signer.sign(invalidData, duration, TokenMode.uuid));
    }

    @Test
    public void verify_valid_token_should_returns_payload() throws InterruptedException {
        UserData data = new UserData("john.doe@example.com");
        String token = signer.sign(data, Duration.ofHours(2), TokenMode.uuid); // Generate a valid token
    ...
    public void verify_expired_token_should_throws_exception() throws InterruptedException {
        UserData data = new UserData("john.doe@example.com");
        String token = signer.sign(data, Duration.ofMillis(1), TokenMode.uuid); // Token with short duration
